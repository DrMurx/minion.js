...
t.test('SQL', t => {
  ...
  t.test('Custom placeholder', t => {
    t.same(Statement.sql`SELECT 1`.toQuery({placeholder: '?'}), {text: 'SELECT 1', values: []});
    t.same(Statement.sql`SELECT ${1};`.toQuery({placeholder: '?'}), {text: 'SELECT ?;', values: [1]});
    t.same(Statement.sql`SELECT ${1}, ${'2'}, ${[3]};`.toQuery({placeholder: '?'}), {
      text: 'SELECT ?, ?, ?;',
      values: [1, '2', [3]]
    });

    const partial = Statement.sql`AND two = ${'Two'} AND three = ${3}`;
    t.same(Statement.sql`SELECT * FROM foo WHERE one = ${'One'} ${partial}`.toQuery({placeholder: '?'}), {
      text: 'SELECT * FROM foo WHERE one = ? AND two = ? AND three = ?',
      values: ['One', 'Two', 3]
    });
    t.same(
      Statement.sql`SELECT * FROM foo WHERE one = ${'One'} ${partial} ${partial} AND four = ${4}`.toQuery({placeholder: '?'}),
      {
