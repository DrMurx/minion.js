...
t.test('Queue with PostgreSQL backend', skip, async (t) => {
  ...
  await t.test('Repair abandoned job in foreground queue (have to be handled manually)', async (t) => {
    const worker = await queue.getNewWorker().register();
    const jobHandle1 = await queue.addJob('test', {}, { queueName: backend.FOREGROUND_QUEUE });
    ...
    await jobHandle1.sync();
    t.equal(jobHandle1.state, JobState.Running);
    t.equal(jobHandle1.queueName, backend.FOREGROUND_QUEUE);
  ...
  await t.test('Repair old jobs', async (t) => {
    ...
    const finishedAt1 = jobHandle2.finishedAt!.getMilliseconds();
    ...
    const finishedAt2 = jobHandle3.finishedAt!.getMilliseconds();
    await pool.query(`UPDATE ${JOB_TABLE} SET finished_at = TO_TIMESTAMP($1) WHERE id = $2`, [
  ...
  await t.test('Delayed jobs', async (t) => {
    ...
    await pool.query(`UPDATE ${JOB_TABLE} SET delay_until = NOW() - INTERVAL '1 day' WHERE id = $1`, [jobHandle1.id]);
    const job2 = (await worker.getNextExecutor())!;
  ...
  await t.test('Nested data structures', async (t) => {
    ...
    t.same(jobHandle1.metadata, { foo: [4, 5, 6, 7], bar: { baz: [1, 2, 3] }, baz: 'yada', yada: ['works'], so: true });
  ...
  await t.test('Multiple attempts with backoff while processing', async (t) => {
    ...
    await pool.query(`UPDATE ${JOB_TABLE} SET delay_until = NOW() WHERE id = $1`, [jobHandle1.id]); // Skip backoff
    ...
    await pool.query(`UPDATE ${JOB_TABLE} SET delay_until = NOW() WHERE id = $1`, [jobHandle1.id]); // Skip backoff again
  ...
  await t.test('Expiring jobs', async (t) => {
    ...
    t.equal(await queue.listJobInfos({ states: [JobState.Pending] }).numRows(), 1);
    await pool.query(`UPDATE ${JOB_TABLE} SET expires_at = NOW() - INTERVAL '1 day' WHERE id = $1`, [jobHandle3.id]);
    await queue.prune();
    t.notOk(await worker.getNextExecutor());
    ...
    t.equal(job4.id, jobHandle4.id);
    t.ok(await job4.markSucceeded());
    await pool.query(`UPDATE ${JOB_TABLE} SET expires_at = NOW() - INTERVAL '1 day' WHERE id = $1`, [jobHandle4.id]);
    await queue.prune();
    ...
    await pool.query(`UPDATE ${JOB_TABLE} SET expires_at = NOW() - INTERVAL '1 day' WHERE id = $1`, [jobHandle5.id]);
    await queue.prune();
    await jobHandle5.sync();
    ...
    await pool.query(`UPDATE ${JOB_TABLE} SET expires_at = NOW() - INTERVAL '1 day' WHERE id = $1`, [jobHandle6.id]);
    await queue.prune();
    ...
    await pool.query(`UPDATE ${JOB_TABLE} SET expires_at = NOW() - INTERVAL '1 day' WHERE id = $1`, [jobHandle7.id]);
    await queue.prune();
  ...
  await t.test('runJob', async (t) => {
    ...
    const jobHandle2 = await queue.addJob('test');
    const jobHandle3 = await queue.addJob('test', {}, { parentJobIds: [jobHandle1.id, jobHandle2.id] });
    t.notOk(await queue.runJob(jobHandle3.id));
    ...
    t.equal(jobHandle1.maxAttempts, 2);
    t.equal(jobHandle1.attempt, 1);
    t.ok(await queue.runJob(jobHandle1.id));
    ...
    t.ok(await queue.runJob(jobHandle2.id));
    await jobHandle2.sync();
    t.equal(jobHandle2.queueName, backend.FOREGROUND_QUEUE);
    t.equal(jobHandle2.state, JobState.Succeeded);
    t.equal(jobHandle2.maxAttempts, 2);
    t.equal(jobHandle2.attempt, 2);

    t.ok(await queue.runJob(jobHandle3.id));
    await jobHandle3.sync();
    t.equal(jobHandle3.queueName, backend.FOREGROUND_QUEUE);
    t.equal(jobHandle3.state, JobState.Succeeded);
    t.equal(jobHandle3.maxAttempts, 3);
    t.equal(jobHandle3.attempt, 3);

    t.notOk(await queue.runJob(jobHandle3.id + 1));

    const jobHandle4 = await queue.addJob('fail');
    let result;
    try {
      await queue.runJob(jobHandle4.id);
    } catch (error) {
      result = error;
    }
    ...
    t.equal(jobHandle4.state, JobState.Failed);
    t.equal(jobHandle4.queueName, backend.FOREGROUND_QUEUE);
