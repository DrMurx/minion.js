...
const SCHEMA = 'queue_pg_backend_test';
...
t.test('PostgreSQL backend', skip, async (t) => {
  ...
  await t.test('Get job infos', async (t) => {
    ...
    const jobHandle1 = await queue.addJob('add');
    const jobHandle2 = await queue.addJob('fail', {}, { maxAttempts: 5 });
    const jobHandle3 = await queue.addJob('test', {}, { queueName: 'another_queue' });
    ...
    t.equal(batch1[0].id, jobHandle1.id);
    ...
    t.equal(batch1[1].id, jobHandle2.id);
    ...
    t.equal(batch8[0].id, jobHandle3.id);
    t.notOk(batch8[1]);
  });
